...
To use the currently supported middlewares with this model, there is one important caveat to keep in mind when developing your Serverless application. In a Lambda function, Flask and Django create subsegments instead of segments when an endpoint is reached because `Segments` cannot be generated by the Lambda function. It is important therefore to make sure that your methods only generate `Subsegments` and not `Segments`

Please follow the steps in the following guide for an example of setting up the a Serverless application that utilizes API Gateway and Lambda:

[Placeholder Link](https://aws.amazon.com)
